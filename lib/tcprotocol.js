...
function loadFriends() {
    console.log("Loading existing friends");
    var that = this;
    for (var i = 0; i < that.conf.buddies.length; i++) {
        ...
        outboundConnection(buddy, function(err, connection) {
            if (err) {
                console.log("Unable to connect to buddy " + buddy.address);
                return;
            }
            console.log("Adding buddy " + buddy.address);
            that.buddyManager.addBuddy(buddy);
            that.sendPing(buddy);
            that.sendClient(buddy);
            that.sendVersion(buddy);
            that.sendProfileName(buddy);
            that.sendProfileText(buddy);
            that.sendAddMe(buddy);
            that.sendStatus(buddy, 'available');
        });
    }
};
...
TCProtocol.prototype.sendPing = function(buddy) {
    ...
    if (buddy.outboundConnection.isConnected) {
        buddy.outboundConnection.socket.write('ping ' + this.conf.profile.address + ' ' + buddy.authToken + '\n');
    } else {
        console.log("Unable to ping no outbound connection in place");
    }
};
...
TCProtocol.prototype.sendClient = function(buddy) {
    if (buddy.outboundConnection.isConnected) {
        buddy.outboundConnection.socket.write('client ' + this.conf.client.name + '\n');
    }
};
...
TCProtocol.prototype.sendVersion = function(buddy) {
    if (buddy.outboundConnection.isConnected) {
        buddy.outboundConnection.socket.write('version ' + this.conf.client.version + '\n');
    }
};
...
TCProtocol.prototype.sendStatus = function(buddy, status) {
    if (buddy.outboundConnection.isConnected) {
        buddy.outboundConnection.socket.write('status ' + status + '\n');
    }
};
...
TCProtocol.prototype.sendProfileName = function(buddy) {
    if (buddy.outboundConnection.isConnected) {
        buddy.outboundConnection.socket.write('profile_name ' + this.conf.profile.name + '\n');
    }
};
...
TCProtocol.prototype.sendProfileText = function(buddy) {
    if (buddy.outboundConnection.isConnected) {
        buddy.outboundConnection.socket.write('profile_text ' + this.conf.profile.text + '\n');
    }
};
...
TCProtocol.prototype.sendMessage = function(buddy, message) {
    if(buddy.outboundConnection.isConnected) {
        buddy.outboundConnection.socket.write('message ' + message + '\n');
    }
};
