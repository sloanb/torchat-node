...
function loadFriends(callback) {
    ...
    for (var i = 0; i < that.conf.buddies.length; i++) {
        ...
        //     } else {
        //         that.buddyManager.addBuddy(b);
        //         // setTimer(that.sendPing(b), 10000);
        //         // that.sendClient(b);
        //         // that.sendVersion(b);
        //         // that.sendProfileName(b);
        //         // that.sendProfileText(b);
...
var inboundConnection = function(options) {
    ...
    function onConnectionEnd() {
        log.error("Inbound connection for " + _buddy.address + " disconnected");
        _buddy.status = 'offline';
        _isConnected = false;
        _socket = null;
    };
...
var outboundConnection = (function() {
    ...
    /**
     * Only a file protocol is allowed to come inbound on the
     * duplex socket connection for outbound communication
     */
    var onDataRead = function() {
        var chunk;
        while (null !== (chunk = _socket.read())) {
        ...
        socks.connection({
            ...
            function(err, socket) {
                if (!err) {
                    ...
                    _socket.on('end', onConnectionEnd);
                    _socket.on('readable', onDataRead);
                } else {
                    _isConnected = false;
                    _socket = null;
                    callback(err, null);
